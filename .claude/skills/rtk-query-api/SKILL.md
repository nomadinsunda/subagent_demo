...
triggers:
  # 1. API 작업 의도 (핵심)
  - intent: ["API 생성", "API 수정", "엔드포인트", "데이터 요청", "백엔드 연동"]

  # 2. RTK Query 관련 키워드
  - keywords: ["rtk query", "createApi", "injectEndpoints"]

  # 3. HTTP / 네트워크
  - keywords: ["api", "fetch", "axios", "http", "요청", "응답"]

  # 4. 캐시/태그 시스템
  - keywords: ["providesTags", "invalidatesTags", "캐시", "tag"]

  # 5. 엔드포인트 정의
  - keywords: ["query", "mutation", "endpoint"]

  # 6. 인증 연동 (withReauth)
  - keywords: ["401", "refresh"]
...
3. **withReauth 필수** — `baseQuery`는 반드시 `withReauth`로 감싸야 함 (**withReauth** — tech-standards)
4. **No Axios** — 모든 HTTP 통신은 RTK Query만 사용 (**No Axios** — tech-standards)
