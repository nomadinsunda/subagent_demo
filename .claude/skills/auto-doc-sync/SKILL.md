---
name: auto-doc-sync
description: 문서 자동 동기화 규칙. docs/*.md 업데이트 트리거 조건, 작업 전 필독 순서, 변경 보고 형식, 신규 도메인 docs 생성 기준. 비즈니스 로직 변경 시 참조.
user-invocable: false

triggers:
  # 1. 비즈니스 로직 변경 (핵심)
  - intent: ["정책 변경", "비즈니스 로직", "상태값 변경", "계산 변경"]

  # 2. 도메인 작업 전반
  - intent: ["주문", "상품", "포인트", "리뷰", "문의", "회원"]

  # 3. 상수/계산 관련 (중요)
  - keywords: ["constants", "계산", "비율", "threshold", "rate"]
...
`.claude/rules/doc-sync.md`의 구체적 실행 방법을 정의한다.  
...
| **Docs First** | 작업 전 docs/*.md와 CLAUDE.md 먼저 읽기 | 명세와 다른 구현 위험 |
...
```bash
bash .claude/skills/auto-doc-sync/scripts/validate-docs.sh
```
