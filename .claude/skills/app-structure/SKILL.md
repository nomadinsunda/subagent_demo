...
triggers:
  # 1. 구조/설계 관련 의도 (핵심)
  - intent: ["구조 설계", "디렉토리", "폴더 구조", "아키텍처", "구성"]

  # 2. 페이지/컴포넌트 생성
  - intent: ["페이지 생성", "컴포넌트 생성", "화면 추가", "UI 구성"]

  # 3. 라우팅 관련 작업
  - keywords: ["route", "routing", "라우팅", "navigate", "경로", "url"]

  # 4. Feature 기반 도메인 작업
  - keywords: ["feature", "domain", "모듈", "기능 추가"]
  ...
  # 6. 특정 구조 키워드
  - keywords: ["MyPage", "ProtectedRoute", "Layout", "Outlet"]

  # 7. Redux 구조 관련
  - keywords: ["store", "slice", "redux", "state 구조"]
---
