...
triggers:
  # 1. Mock / 테스트 환경 관련 의도 (핵심)
  - intent: ["mock", "테스트 데이터", "가짜 데이터", "mock api"]

  # 2. API 구현 시 자동 개입 (중요)
  - intent: ["API 생성", "엔드포인트 추가", "백엔드 없이", "데이터 요청 구현"]

  # 3. mockBaseQuery 직접 키워드
  - keywords: ["mockBaseQuery", "handleRequest", "라우팅"]

  # 4. 데이터 변경 관련 (가변/불변 규칙)
  - keywords: ["데이터 수정", "배열 수정", "원본 배열"]
  ...
  # 7. 도메인 추가 (핵심)
  - intent: ["도메인 추가", "기능 추가", "새 API", "새 데이터"]
---
