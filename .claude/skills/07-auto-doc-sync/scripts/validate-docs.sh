...
# 사용법: bash .claude/skills/07-auto-doc-sync/scripts/validate-docs.sh
...
if [[ $FAIL -gt 0 ]]; then
  echo ""
  echo "❌ 실패 항목이 있습니다."
  echo "   누락된 docs/*.md를 생성하거나 07-auto-doc-sync/template.md를 참고하여 문서를 작성하세요."
  echo ""
  echo "   실행: bash .claude/skills/07-auto-doc-sync/scripts/validate-docs.sh"
  exit 1
elif [[ $WARN -gt 0 ]]; then
  echo ""
